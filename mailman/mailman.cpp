...
std::string BuildSender(const std::string& sender)
{
    ...
    const std::string name =
        pos != sender.npos ? sender.substr(0, pos) : sender;
    return "From: \"" + name + "\" <" + sender + ">\r\n";
}
    ...
    try {
        ...
        CryptoPP::Base64Encoder* b64_enc(
            new CryptoPP::Base64Encoder(
                string_sink, false/* We don't need '\n' at the back. */));
                                          ...
                                          const std::string& message)
{
    const std::string payload_text[] = {
        ...
        // The following commented code is copied directly from the raw content
        // generated by a mail software. It is soon turned out that some smtp
        // server(e.g. smtp.163.com) doesn't need these stuff at all.
        //
        //"X-Priority: 3\r\n",
        //"X-Has-Attach: no\r\n",
        //"X-Mailer: libcurl\r\n",
...
void DecodeMail()
{
    ...
    const int kIVCipherSize = 24;
    std::string cipher = "";

    std::string iv_cipher = cipher.substr(0, kIVCipherSize);
    std::vector<byte> iv_decoded(kIVCipherSize / 4 * 3);
    CryptoPP::ArraySink* array_sink(
        new CryptoPP::ArraySink(&iv_decoded[0], iv_decoded.size()));
    CryptoPP::Base64Decoder* b64_dec(
        new CryptoPP::Base64Decoder(array_sink));
    CryptoPP::StringSource source(iv_cipher, true, b64_dec);

    std::string message_cipher_b64 = cipher.substr(kIVCipherSize, -1);
    std::vector<byte> message_cipher(
        message_cipher_b64.size() / 4 * 3 / CryptoPP::AES::BLOCKSIZE *
        CryptoPP::AES::BLOCKSIZE);
    CryptoPP::ArraySink* array_sink2(
        new CryptoPP::ArraySink(&message_cipher[0], message_cipher.size()));
    CryptoPP::Base64Decoder* b64_dec2(
        new CryptoPP::Base64Decoder(array_sink2));
    CryptoPP::StringSource source2(message_cipher_b64, true, b64_dec2);

    std::string message;
    CryptoPP::StringSink* string_sink(new CryptoPP::StringSink(message));
    CryptoPP::CBC_Mode<CryptoPP::AES>::Decryption dec(key, key.size(),
                                                      &iv_decoded[0]);
    CryptoPP::StreamTransformationFilter* trans(
        new CryptoPP::StreamTransformationFilter(dec, string_sink));
    CryptoPP::ArraySource source3(&message_cipher[0], message_cipher.size(),
                                  true, trans);
}
